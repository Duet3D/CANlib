/*
 * RRF3Common.cpp
 *
 *  Created on: 18 Oct 2021
 *      Author: David
 */

#include "RRF3Common.h"
#include <General/Bitmap.h>

// Append the driver status as text. 'severity' meaning is 0 for all, 1 for warnings/errors, 2 for errors only.
// If nothing to print, append "ok".
void StandardDriverStatus::AppendText(const StringRef& str, unsigned int severity) const noexcept
{
	// Report any errors
	const uint32_t relevantBits = (severity == 2) ? all & ErrorMask
									: (severity == 1) ? all & (WarningMask | ErrorMask)
										: all & (WarningMask | ErrorMask | InfoMask);
	if (relevantBits == 0)
	{
		str.cat("ok");
	}
	else
	{
		Bitmap<uint32_t> errors(relevantBits);
		errors.Iterate([&str](unsigned int bitNum, unsigned int index) noexcept -> void
						{
							if (index != 0)
							{
								str.cat(", ");
							}
							str.cat(BitMeanings[bitNum]);
						}
					);
	}
}

MinCurMax& MinCurMax::operator=(const ShortMinCurMax& arg) noexcept
{
	minimum = (float)arg.minimum;
	current = (float)arg.current;
	maximum = (float)arg.maximum;
	return *this;
}

ShortMinCurMax& ShortMinCurMax::operator=(const MinCurMax& arg) noexcept
{
	minimum = (float16_t)arg.minimum;
	current = (float16_t)arg.current;
	maximum = (float16_t)arg.maximum;
	return *this;
}

ShortPressureAdvanceParameters::ShortPressureAdvanceParameters(const PressureAdvanceParameters& p) noexcept
	: dk((float16_t)p.dk)
{
	k[0] = (float16_t)p.k[0];
	k[1] = (float16_t)p.k[1];
}

PressureAdvanceParameters::PressureAdvanceParameters(float k0) noexcept
	: dk(std::numeric_limits<float>::infinity())
{
	k[0] = k[1] = k0;
}

PressureAdvanceParameters::PressureAdvanceParameters(const ShortPressureAdvanceParameters& p) noexcept
	: dk(p.dk)
{
	k[0] = p.k[0];
	k[1] = p.k[1];
}

// End
